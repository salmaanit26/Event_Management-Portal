...
if ($_POST && isset($_POST['assign_reviewer'])) {
    try {
        ...
        $stmt->execute([$reviewer_id, $registration_id]);
        
        $success_message = "Reviewer assigned successfully!";
    } catch (Exception $e) {
        $error_message = "Error assigning reviewer: " . $e->getMessage();
    }
}

// Get all reviewers
$reviewers = $conn->query("SELECT id, full_name, department FROM users WHERE role = 'reviewer' ORDER BY full_name")->fetchAll();

// Get all IRA registrations that need reviewer assignment
$registrations = $conn->query("
    SELECT r.*, e.event_name, s.slot_date, s.slot_time, s.hall_name, u.full_name as reviewer_name
...
<head>
    ...
    <title>Manage Reviewers - Admin Panel</title>
...
<body>
    ...
    <div class="container">
        <div class="page-header">
            ...
            <p>Assign reviewers to IRA registrations and manage faculty assignments</p>
        </div>
        ...
        <div class="reviewers-section">
            ...
            <?php if (empty($reviewers)): ?>
                <p>No reviewers found. Please add reviewers to the system first.</p>
            <?php else: ?>
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1rem;">
                    <?php foreach ($reviewers as $reviewer): ?>
                        <div class="reviewer-card">
                            <h4><?php echo htmlspecialchars($reviewer['full_name']); ?></h4>
                            <p><strong>Department:</strong> <?php echo htmlspecialchars($reviewer['department']); ?></p>
                            <p><strong>ID:</strong> <?php echo $reviewer['id']; ?></p>
                        </div>
                    <?php endforeach; ?>
                </div>
            <?php endif; ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if ($reg['reviewer_name']): ?>
                                        <strong><?php echo htmlspecialchars($reg['reviewer_name']); ?></strong>
                                    <?php else: ?>
                                ...
                                <td>
                                    <form method="post" style="display: inline;">
                                        ...
                                        <select name="reviewer_id" required style="margin-right: 0.5rem; padding: 0.25rem;">
                                            <option value="">Select Reviewer</option>
                                            <?php foreach ($reviewers as $reviewer): ?>
                                                <option value="<?php echo $reviewer['id']; ?>" 
                                                    <?php echo ($reg['assigned_reviewer'] == $reviewer['id']) ? 'selected' : ''; ?>>
                                                    <?php echo htmlspecialchars($reviewer['full_name']); ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                        <button type="submit" name="assign_reviewer" class="btn btn-primary">Assign</button>
                                    </form>
                                </td>
                            </tr>
                        <?php endforeach; ?>
        ...
        <div style="background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(10px); border-radius: 16px; padding: 2rem; margin-top: 2rem; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.1);">
            ...
            <ul style="list-style: none; padding: 0;">
                <li style="padding: 0.5rem 0; color: #4a5568;">• <strong>Assign Reviewers:</strong> Select appropriate faculty for each IRA registration</li>
                <li style="padding: 0.5rem 0; color: #4a5568;">• <strong>Faculty Access:</strong> Assigned reviewers will be able to evaluate students</li>
                <li style="padding: 0.5rem 0; color: #4a5568;">• <strong>Department Match:</strong> Consider assigning reviewers from relevant departments</li>
